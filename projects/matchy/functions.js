...
function search(animals, name) {
    for (var i = 0; i < animals.length; i++) {
        console.log(animals[i], 'current animal[index]')
...
//   - Takes 2 parameter, an Array of animals, iand an Object representing a new animal to be added.
...
function add(animals, animal) {
    ...
    if (animal.name.length > 0 && animal.species.length > 0) {
        for (var i = 0; i < animals.length; i++) {
            if (!unique[animals[i].name]) {
                unique[animals[i].name] = 1;
            }
        }
    }
    var objectKey = Object.keys(unique);
    if (!objectKey.includes(animal.name)) {
        animals.push(animal);
    }
    return null;
}
